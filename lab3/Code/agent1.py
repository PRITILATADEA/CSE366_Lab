...
class Agent1(pygame.sprite.Sprite):
    ...
    def find_nearest_task(self):
        ...
        nearest_task = None
        shortest_path = None
        for task_position in self.environment.task_locations.keys(): #Iterates through all task positions in the environment's task locations dictionary.
    ...
    def find_path_to(self, target):
        ...
        goal = target
        open_set = [] # A priority queue (min-heap) to store positions to be explored, prioritized by their cost (total heuristic).
        heapq.heappush(open_set, (0, start, [start], 0))  # Include accumulated heuristic in the tuple
        g_scores = {start: 0} #A dictionary storing the cost to reach each node from the start node, initialized with start having a cost of 0

        while open_set:#as long as there are nodes in the open_set to explore.
